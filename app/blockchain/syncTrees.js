// path: oudra-server/app/blockchain/syncTrees.js
...
async function enrollWithRetry(wallet, contract, provider, tree, baseGasPrice, nonce, chainId) {
    ...
    try {
      ...
      if (msg.includes("could not replace") || msg.includes("replacement transaction")) {
        ...
        if (existing?.treeId?.length > 0) {
          console.log(`   ✅ Tree already on chain`);
...
async function _enrollExistingTrees(contractAddress) {
  ...
  try {
    ...
    const isAmoy      = chainId === 80002n;
    if (!isLocalhost && !isAmoy)
      throw new Error(`Unexpected chainId: ${chainId}. Expected 80002 or 31337.`);
    console.log(isLocalhost ? "🖥️  localhost (Hardhat)" : "🌐 Amoy testnet");
  } catch (err) {
    if (err.message.includes("MAINNET") || err.message.includes("Unexpected chainId")) throw err;
    throw new Error(`Cannot reach RPC at ${process.env.BLOCKCHAIN_RPC_URL}.`);
  }
  ...
  // ── Fetch ALL pending trees — no batch limit ──────────────────────────────
  ...
  console.log(`🌳 Trees pending sync: ${totalPending} — syncing all of them`);
  if (trees.length === 0) {
  ...
  if (isLocalhost) {
    ...
    console.log(`⛽ Localhost gas price: 1 gwei (instant confirmation)\n`);
  } else {
    try {
  ...
  for (let i = 0; i < trees.length; i++) {
    ...
    try {
      ...
      if (!tree.treeId)        throw new Error("Tree is missing treeId");
      if (!tree.gps || tree.gps.lat === undefined || tree.gps.lng === undefined)
        throw new Error("Missing GPS data");
      if (!tree.lifecycleStatus) throw new Error("Missing lifecycleStatus");
      ...
      const result = await enrollWithRetry(wallet, contract, provider, tree, baseGasPrice, nonce, chainId);
      ...
      if (result.alreadyOnChain) {
        ...
        tree.blockchainStatus = "Verified";
        tree.blockchainTxHash = result.transactionHash; // ✅ fixed: was result.hash
        tree.blockchainBlock  = result.blockNumber;     // ✅ fixed: saves block number too
        await tree.save();
        console.log(`   💾 MongoDB updated — txHash: ${result.transactionHash}\n`);
        successCount++;
      }

      nonce++;

      if (i < trees.length - 1 && !isLocalhost) {
      ...
      if (!isLocalhost) await sleep(5_000);
    }
  }
